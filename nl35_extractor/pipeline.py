"""
pipeline.py — Config-driven NL-6 Commission Schedule extraction pipeline.

Usage:
  python3 pipeline.py                          # incremental (default)
  python3 pipeline.py --force                  # re-extract everything
  python3 pipeline.py --force-company sbi_general
...
from extractor.consolidated_detector import find_nl6_pages, extract_nl6_to_temp
...
def main():
    parser = argparse.ArgumentParser(description="NL-6 Config-Driven Extraction Pipeline")
    parser.add_argument("--force", action="store_true",
                        help="Re-extract all files regardless of processed log")
    parser.add_argument("--force-company", nargs="+", default=None,
                        help="Re-extract only these company keys across all quarters")
    ...
    for result in to_process:
        ...
        try:
            ...
            if result.source_type == "consolidated":
                # Per-company overrides (hard page pins or relaxed min_matches)
                page_overrides = config.get("nl6_page_overrides", {})
                override = page_overrides.get(result.company_key) or {}

                if "start" in override:
                    # Hard-pinned page range (1-indexed in config → 0-indexed here)
                    ...
                    keywords = config.get("nl6_keywords", None)
                    min_matches = int(override.get(
                        "min_matches",
                        config.get("nl6_keyword_min_matches", 4),
                    ))
                    pages = find_nl6_pages(result.pdf_path, keywords, min_matches)

                if pages is None:
                    logger.warning(
                        f"  Could not find NL-6 section in consolidated PDF, skipping: "
                        f"{result.pdf_path}"
                    )
                    failed += 1
                    failed_files.append((result.pdf_path, "NL-6 section not found"))
                    continue

                temp_file = extract_nl6_to_temp(result.pdf_path, pages[0], pages[1])
            ...
            # Align CY/PY LOB sets so both periods always have the same LOBs
    ...
    if all_extractions:
        ...
        year_selection = config.get("year_selection", "both")
        save_workbook(all_extractions, master_path, stats=stats, year_selection=year_selection)

        # 2. Validation report CSV (needed by summary/detail writers)
        report_path = os.path.join(
            os.path.dirname(master_path), "validation_report.csv"
        )
