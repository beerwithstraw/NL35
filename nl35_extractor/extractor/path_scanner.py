...
def scan(config: Dict[str, Any]) -> List[ScanResult]:
    ...
    for fy in fiscal_years:
        ...
        for quarter in quarters:
            ...
            # --- Scan NL6/ subfolder: explicitly for NL-6 forms ---
            nl6_path = os.path.join(q_path, "NL6")
            if os.path.isdir(nl6_path):
                for fname in os.listdir(nl6_path):
                    if not fname.lower().endswith(".pdf"):
                        continue
                    result = _extract_company_key(fname)
                    if result:
                        company_key, company_raw = result
                        pdf_path = os.path.join(nl6_path, fname)
            ...
            # --- Scan NL6_Consolidated/ subfolder ---
            consol_path = os.path.join(q_path, "NL6_Consolidated")
            if config.get("consolidated_mode", "dynamic") != "skip" and os.path.isdir(consol_path):
                for fname in os.listdir(consol_path):
                    ...
                    # Skip if direct file already found for this company
                    if company_key in direct_companies:
                        logger.debug(
                            f"Skipping consolidated for {company_key} — "
                            f"direct NL6 file exists"
                        )
                        continue
