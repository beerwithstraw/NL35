"""
path_scanner.py — walks the folder structure and returns PDFs to process.
Scans NL35/ subfolder within each quarter directory.
"""
...
def scan(config: Dict[str, Any]) -> List[ScanResult]:
    ...
    for fy in fiscal_years:
        ...
        for quarter in quarters:
            ...
            # Scan NL35/ subfolder
            nl35_path = os.path.join(q_path, "NL35")
            if os.path.isdir(nl35_path):
                for fname in os.listdir(nl35_path):
                    if not fname.lower().endswith(".pdf"):
                        continue
                    result = _extract_company_key(fname)
                    if result:
                        company_key, company_raw = result
                        pdf_path = os.path.join(nl35_path, fname)
