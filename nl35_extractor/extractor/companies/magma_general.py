"""
Parser for Magma General Insurance Limited (NL-6 Commission Schedule).

PDF Structure: 2 pages, 4 tables each.
  Page 1: CY (Q3 2025-26)
  Page 2: PY (Q3 2024-25)

  Per page:
    T0: top section, LOBs fire→total_health (23c, 11 LOBs × 2 cols)
    T1: channel section for T0 LOBs — start_in_channel_section=True
    T2: top section, LOBs wc_el→grand_total (17c, 8 LOBs × 2 cols)
...
2-row LOB header: r0=category span ("Miscellaneous"), r1=sub-LOBs.
  ...
  "Crop" (bare) alias added to HEADER_TO_LOB → crop_insurance.

CY/PY: detect_period_year gives 2025 (P1) / 2024 (P2).
"""
...
_FALLBACK_NAME = "Magma General Insurance Limited"


def parse_magma_general(
    pdf_path: str,
    company_key: str,
    quarter: str = "",
    year: str = "",
) -> CompanyExtract:
    logger.info(f"Parsing Magma General NL-6 PDF: {pdf_path}")
    ...
    if not page_data:
        logger.warning("Magma General: no valid pages found")
